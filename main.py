...
def evaluate(val_loader, model, criterion, epoch, train_loss, start, log):
    ...
    with torch.no_grad():
        for i, (images, target, fnames) in enumerate(val_loader):
            ...
            message_val = format_log_message("val", i, epoch, train_loss[0], map.avg, valid_acc1, valid_acc5, time_to_str(timer() - start, "sec"))
            print(f'\r{message_val}', end='', flush=True)
        
        message_val_epoch = format_log_message("val", i, epoch, train_loss[0], map.avg,valid_acc1, valid_acc5, time_to_str(timer() - start, "sec"))
        log.write("\n")  
...
def main():
    ...
    train_imlist = pd.read_csv(args.train_datacsv) # need to check what does it print as the dataset is arranged with knife name , index number.
    train_gen = knifeDataset(train_imlist, mode="train")
    train_loader = DataLoader(train_gen, batch_size=args.batch_size, shuffle=True, pin_memory=True, num_workers=args.num_workers) #The DataLoader uses a dataset object (e.g., knifeDataset) as input and retrieves individual samples using the dataset's __getitem__ method. It then combines these samples into batches for model consumption.
    ...
    ############################# Parameters #################################
    optimizer = init_optimizer(model, **optimizer_kwargs(args)) #It adjusts the model's weights during training to minimize the loss function.

    scheduler = init_lr_scheduler(optimizer, **lr_scheduler_kwargs(args)) #A scheduler adjusts the learning rate during training based on a predefined strategy.

    
    criterion = nn.CrossEntropyLoss().cuda() #CrossEntropyLoss is a loss function often used for multi-class classification tasks.
